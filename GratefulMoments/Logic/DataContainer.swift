...
// Marcar la clase `DataContainer` con `@MainActor` garantiza que cualquier interacción con el contenedor desde tus vistas se realice en el hilo principal, lo cual es necesario para las actualizaciones de la interfaz de usuario

// La macro `@Observable` le indica a SwiftUI que supervise tu `DataContainer` en busca de cambios. Agregas el contenedor al entorno para que el contenedor del modelo y cualquier propiedad futura de `DataContainer` estén disponibles a través de la jerarquía de vistas.
